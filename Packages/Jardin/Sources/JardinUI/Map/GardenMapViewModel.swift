...
public final class GardenMapViewModel: ObservableObject {
    ...
    @Published private var gestureScaleBase: CGFloat = 1
    ...
    public func applyMagnification(_ value: CGFloat) {
        scale = min(max(gestureScaleBase * value, 0.5), 4)
    }
    ...
    public func zoom(by factor: CGFloat) {
        withAnimation(.spring(duration: 0.25)) {
            scale = min(max(scale * factor, 0.5), 4)
        }
        gestureScaleBase = scale
    }
    ...
    /// Rectangle prédéfini au centre (l'utilisateur l'ajuste ensuite point par point).
